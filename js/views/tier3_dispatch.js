...
window.RakshakTier3DispatchView = {
  ...
  assignedUnitId: null,
  pcrDispatched: false, // PCR only moves after operator explicitly dispatches
  ...
  render: function(container) {
    ...
    container.innerHTML = `
      ...
      <div class="card" style="margin-bottom: 16px;">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          ...
          <div style="display: flex; align-items: center; gap: 10px;">
            <span id="map-tracking-indicator" class="mono text-muted" style="font-size: 11px; color: #D97706; font-weight: 600;">⚠ AWAITING DISPATCH</span>
            <button id="btn-dispatch-and-track"
              onclick="window.RakshakTier3DispatchView.dispatchAndTrack()"
              style="display: inline-flex; align-items: center; gap: 6px; background: #0F4C81; color: #FFFFFF; border: none; border-radius: 3px; padding: 6px 14px; font-size: 12px; font-weight: 700; cursor: pointer; letter-spacing: 0.3px; box-shadow: 0 2px 6px rgba(15,76,129,0.35);">
              ...
              Dispatch &amp; Track ${leadUnit.id}
            </button>
            <button id="btn-cancel-dispatch"
              onclick="window.RakshakTier3DispatchView.cancelDispatch()"
              style="display: none; align-items: center; gap: 6px; background: #DC2626; color: #FFFFFF; border: none; border-radius: 3px; padding: 6px 14px; font-size: 12px; font-weight: 700; cursor: pointer; letter-spacing: 0.3px; box-shadow: 0 2px 6px rgba(220,38,38,0.35);">
        ...
        <div class="card-body-flush">
          <div class="map-container-wrapper" style="height: 400px;">
            ...
            <div class="map-floating-legend">
              ...
              <div class="legend-item"><span class="legend-icon" style="background: #475569;"></span> Fleet Patrol Units (&lt; 15 km)</div>
    ...
    // Initialize Leaflet Map with idle PCR positions — movement starts only after Dispatch button
    this.pcrDispatched = false;
    setTimeout(() => {
      window.RakshakTier3DispatchView.initRouteMap();
      window.RakshakTier3DispatchView.renderIdleFleetOnMap();
    }, 60);
  },
  ...
  // Show all PCR units as static idle markers on map before dispatch
  renderIdleFleetOnMap: function() {
    ...
    // Fit map to show all units + incident
    const allCoords = respondingUnits.map(u => [u.lat, u.lng]);
    allCoords.push(citizen.coords);
    if (allCoords.length > 1) {
      this.routeMapInstance.fitBounds(L.latLngBounds(allCoords), { padding: [40, 40], maxZoom: 14 });
    }

    // Draw 15 km perimeter
    L.circle(citizen.coords, {
      radius: 15000, color: "#0F4C81", weight: 1.5,
      dashArray: "6, 6", fillColor: "#0F4C81", fillOpacity: 0.04
    }).bindPopup("<strong>15 km Tactical Perimeter</strong><br/>Dial 112 ERSS Sector Coverage").addTo(this.broadcastLayersGroup);

    // Incident marker
    const incIcon = L.divIcon({
      html: `<div style="background-color:#C0392B;border:2px solid #FFFFFF;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;color:#FFFFFF;font-size:11px;box-shadow:0 2px 5px rgba(0,0,0,0.3);"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div>`,
      className: "custom-inc-marker", iconSize: [28, 28], iconAnchor: [14, 14]
    });
    L.marker(citizen.coords, { icon: incIcon }).bindPopup(`<strong>Incident: ${citizen.name}</strong><br/>${citizen.area}`).addTo(this.broadcastLayersGroup);

    // All idle PCR markers
    respondingUnits.forEach(u => {
      const isLead = u.id === leadUnit.id;
      const pcrIcon = L.divIcon({
        html: `<div style="background-color:${isLead ? '#0F4C81' : '#475569'};border:2px solid #FFFFFF;border-radius:3px;padding:3px 7px;display:inline-flex;align-items:center;gap:4px;color:#FFFFFF;font-weight:700;font-size:10px;font-family:monospace;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.3);opacity:0.85;">${isLead ? '★' : '◆'} ${u.id}</div>`,
        className: "custom-pcr-marker", iconSize: [52, 22], iconAnchor: [26, 11]
      });
      L.marker([u.lat, u.lng], { icon: pcrIcon })
        .bindPopup(`<strong>${u.id} (${u.callsign})</strong><br/>${u.officer}<br/>${u.distanceKm} km away &bull; ETA ~${u.etaMinutes} min<br/><em style="color:#64748B;">Awaiting dispatch order</em>`)
        .addTo(this.broadcastLayersGroup);
    });

    // Update map indicator label
    const indicatorEl = document.getElementById("map-tracking-indicator");
    if (indicatorEl) indicatorEl.innerHTML = `<span style="color:#D97706;font-weight:700;">⚠ AWAITING DISPATCH — Click "Dispatch & Track" to mobilise ${leadUnit.id}</span>`;
  },

  // Called by the Dispatch & Track button — sets dispatched flag and starts PCR movement
  dispatchAndTrack: function() {
    if (this.pcrDispatched) return; // already dispatched
    this.pcrDispatched = true;

    const citizen = this.getActiveCitizen();
    const allUnits = this.getUnitsWithin15Km(citizen.coords);
    const leadUnit = allUnits.find(u => u.id === this.assignedUnitId) || allUnits.filter(u => u.isWithin15Km)[0] || this.pcrFleet[0];

    // Update dispatch button to show dispatched state
    const btn = document.getElementById("btn-dispatch-and-track");
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> ${leadUnit.id} DISPATCHED — En Route`;
    ...
    // Show Cancel Dispatch button
    const cancelBtn = document.getElementById("btn-cancel-dispatch");
    if (cancelBtn) cancelBtn.style.display = "inline-flex";

    if (window.RakshakApp && window.RakshakApp.playChime) window.RakshakApp.playChime();

    this.logSystemEvent(`Operator confirmed dispatch order. ${leadUnit.id} (${leadUnit.callsign}, ${leadUnit.officer}) mobilised — now en route to incident. ETA: ${leadUnit.etaMinutes} min.`, "dispatched");
    if (window.RakshakAudit && window.RakshakAudit.log) {
      ...
      window.RakshakAudit.log("DISPATCH_SOS", offId, "CAD-DISPATCH-CONSOLE", `Operator-confirmed dispatch: ${leadUnit.id} (${leadUnit.callsign}) mobilised to incident at [${citizen.coords[0].toFixed(4)}, ${citizen.coords[1].toFixed(4)}].`);
    }
  ...
  // Cancel an active dispatch — stops movement, resets map to idle state
  cancelDispatch: function() {
    if (!this.pcrDispatched) return;
    ...
    this.cleanupTrackingIntervals();
    this.pcrDispatched = false;
    this.activeUnitStatus = "STANDBY";
    this.currentLiveDistanceKm = null;
    this.currentLiveEtaMin = null;

    // Reset Dispatch button
    const btn = document.getElementById("btn-dispatch-and-track");
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg> Dispatch &amp; Track ${leadUnit.id}`;
    ...
    if (statusBadge) {
      ...
      statusBadge.innerHTML = `<span class="status-dot" style="background:#94A3B8;"></span> DISPATCH CANCELLED`;
    }

    // Reset ETA clock displays
    ...
    if (distEl) distEl.textContent = `${leadUnit.distanceKm} km`;
    if (elapsedEl) elapsedEl.textContent = "Elapsed: 00:00";

    // Log cancellation
    this.logSystemEvent(`⚠ Dispatch CANCELLED by operator. ${leadUnit.id} (${leadUnit.callsign}) recalled — unit returning to sector patrol. Incident remains open.`, "normal");
    if (window.RakshakAudit && window.RakshakAudit.log) {
      const offId = (window.RakshakAuth && window.RakshakAuth.getCurrentUser) ? window.RakshakAuth.getCurrentUser().officerId : "OD-CP-1023";
      window.RakshakAudit.log("DISPATCH_CANCEL", offId, "CAD-DISPATCH-CONSOLE", `Operator cancelled dispatch of ${leadUnit.id} (${leadUnit.callsign}). Unit recalled to sector patrol.`);
    }

    // Restore idle fleet view on map
    this.renderIdleFleetOnMap();
  },
  ...
  dispatchNearestPcr: function() {
    ...
    this.pcrDispatched = true;
    const btn = document.getElementById("btn-dispatch-and-track");
    if (btn) { btn.disabled = true; btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> ${leadUnit.id} DISPATCHED`; btn.style.background = "#16A34A"; }
    this.startLiveUnitTracking(leadUnit, citizen);
  ...
  dispatchIndividualUnit: function(unitId) {
    ...
    this.renderFleetCards();
    this.pcrDispatched = true;
    const btnI = document.getElementById("btn-dispatch-and-track");
    if (btnI) { btnI.disabled = true; btnI.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> ${unit.id} DISPATCHED`; btnI.style.background = "#16A34A"; }
