...
window.RakshakAuth = {
  ...
  startCameraBiometricVerification: function(officer) {
    ...
    modal.style.display = "flex";
    // Reset inactivity timer so the 120s watchdog doesn't kill the biometric session
    this.state.lastActivityTime = Date.now();
    modal.innerHTML = `
      <div class="camera-biometric-modal-window" onclick="event.stopPropagation()">
        ...
        <div class="cad-camera-container">
          ...
          <div class="cad-camera-overlay">
            ...
            <div id="login-face-not-detected-label" class="face-not-detected-overlay">
    ...
    const faceNotDetectedLabel = document.getElementById("login-face-not-detected-label");
    ...
    const runDetectionLoop = () => {
      ...
      this.detectionInterval = setInterval(() => {
        ...
        // ── Stream health guard ───────────────────────────────────────────────
        // If the camera track was killed by the browser (tab switch, OS, etc.)
        // restart it silently so the user doesn't have to close and reopen.
        const tracks = this.activeCameraStream ? this.activeCameraStream.getTracks() : [];
        if (tracks.length > 0 && tracks[0].readyState === "ended") {
          if (this.detectionInterval) { clearInterval(this.detectionInterval); this.detectionInterval = null; }
          this.stopCameraStream();
          if (hint) hint.textContent = "Camera interrupted — reconnecting...";
          navigator.mediaDevices.getUserMedia({ video: { width: 320, height: 240, facingMode: "user" } })
            .then(newStream => {
              this.activeCameraStream = newStream;
              if (video) { video.srcObject = newStream; video.play().catch(() => {}); }
              runDetectionLoop();
            })
            .catch(() => {
              if (fallbackBox) fallbackBox.style.display = "block";
              if (statusTitle) statusTitle.textContent = "Camera stream lost — please retry";
            });
          return;
        }
        ...
        if (currentStage === "face") {
          ...
          if (det.state === "NO_FACE") {
            if (guide) guide.className = "camera-guide-face no-face";
            if (hint) hint.textContent = "No Face Detected — Position your face within the frame";
            if (faceNotDetectedLabel) faceNotDetectedLabel.style.display = "flex";
            if (statusTitle) {
          ...
          if (det.state === "INSIDE_BOX") {
            ...
            if (guide) guide.className = "camera-guide-face aligned";
            if (faceNotDetectedLabel) faceNotDetectedLabel.style.display = "none";
            faceAlignedFrames++;
            ...
            if (faceAlignedFrames >= REQUIRED_FACE_FRAMES) {
              ...
              setTimeout(() => {
                ...
                if (faceNotDetectedLabel) {
                  faceNotDetectedLabel.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> IRIS NOT DETECTED`;
                  faceNotDetectedLabel.style.display = "flex";
                }
                if (statusTitle) {
          ...
          if (det.state === "NO_FACE") {
            ...
            if (faceNotDetectedLabel) { faceNotDetectedLabel.textContent = ""; faceNotDetectedLabel.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> IRIS NOT DETECTED`; faceNotDetectedLabel.style.display = "flex"; }
            if (statusTitle) {
          ...
          if (det.state === "OUT_OF_BOX") {
            if (guide) guide.className = "camera-guide-iris misaligned";
            if (hint) hint.textContent = "Look directly into the camera";
            if (faceNotDetectedLabel) { faceNotDetectedLabel.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> IRIS NOT DETECTED`; faceNotDetectedLabel.style.display = "flex"; }
            if (statusTitle) {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices.getUserMedia({ video: { width: 320, height: 240, facingMode: "user" } })
        .then(stream => {
          ...
          // ── Visibility Recovery ───────────────────────────────────────────
          // When the user switches tabs or leaves the page, browsers may kill
          // the camera track. When they return, detect this and restart.
          const onVisibilityChange = () => {
            const bioModal = document.getElementById("camera-biometric-modal");
            if (!bioModal || bioModal.style.display === "none") {
              document.removeEventListener("visibilitychange", onVisibilityChange);
              return;
            }
            if (document.hidden) {
              // Page hidden — pause detection to avoid stale canvas reads
              if (this.detectionInterval) {
                clearInterval(this.detectionInterval);
                this.detectionInterval = null;
              }
            } else {
              // Page visible again — check if stream is still alive
              this.state.lastActivityTime = Date.now(); // keep inactivity watchdog off
              const tracks = this.activeCameraStream ? this.activeCameraStream.getTracks() : [];
              const trackAlive = tracks.length > 0 && tracks[0].readyState === "live";
              if (!trackAlive) {
                // Stream died while hidden — restart camera and detection loop
                this.stopCameraStream();
                navigator.mediaDevices.getUserMedia({ video: { width: 320, height: 240, facingMode: "user" } })
                  .then(newStream => {
                    this.activeCameraStream = newStream;
                ...
                // Stream still alive — just restart detection loop
                runDetectionLoop();
              }
            }
          };
          ...
          // Refresh the inactivity timestamp every 10s while biometric modal
          // is open so the 120s watchdog never locks the screen mid-scan.
          const bioKeepalive = setInterval(() => {
            const bioModal = document.getElementById("camera-biometric-modal");
            if (!bioModal || bioModal.style.display === "none") {
              clearInterval(bioKeepalive);
              return;
            }
            this.state.lastActivityTime = Date.now();
          }, 10000);
